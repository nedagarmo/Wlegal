...
extract($_REQUEST);
include_once(dirname(__FILE__) . '/../data.access.objects/subs.dao.class.php');
...
switch($action){
	case "create":
		$result = $dao->insert_sub($nombre, $descripcion);
		if($result){
			$jtable_result['Result'] = "OK";
			$jtable_result['Record'] = $dao->get_sub($nombre)->fields;
		} else {
		...
		break;
	case "update":
		$result = $dao->update_sub($id, $nombre, $descripcion);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
	...
	case "search":
		if(empty($name)){
			$result = $dao->get_all_subs($jtSorting, $jtStartIndex, $jtPageSize);
		}else{
			$result = $dao->get_sub_search($name, $jtSorting, $jtStartIndex, $jtPageSize);
		}
		...
		if($result != null){
			...
			$jtable_result['TotalRecordCount'] = $dao->get_all_count_subs()->fields[0];	
		}else{
			$jtable_result['Result'] = "ERROR";
