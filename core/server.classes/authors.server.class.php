...
extract($_REQUEST);
include_once(dirname(__FILE__) . '/../data.access.objects/authors.dao.class.php');
...
switch($action){
	case "create":
		$result = $dao->insert_author($nombre);
		if($result){
			$jtable_result['Result'] = "OK";
			$jtable_result['Record'] = $dao->get_author($nombre)->fields;
		} else {
		...
		break;
	case "update":
		$result = $dao->update_author($id, $nombre);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
	...
	case "search":
		if(empty($name)){
			$result = $dao->get_all_authors($jtSorting, $jtStartIndex, $jtPageSize);
		}else{
			$result = $dao->get_author_search($name, $jtSorting, $jtStartIndex, $jtPageSize);
		}
		...
		if($result != null){
			...
			$jtable_result['TotalRecordCount'] = $dao->get_all_count_authors()->fields[0];	
		}else{
			$jtable_result['Result'] = "ERROR";
