...
switch($action){
	case "create":
		$result = $dao->insert_relation($nombre);
		if($result){
			$jtable_result['Result'] = "OK";
			$jtable_result['Record'] = $dao->get_relation($nombre)->fields;
		} else {
		...
		break;
	case "update":
		$result = $dao->update_relation($id, $nombre);
		if($result){
			$jtable_result['Result'] = "OK";
		} else {
			$jtable_result['Result'] = "ERROR";
	...
	case "search":
		if(empty($name)){
			$result = $dao->get_all_relations($jtSorting, $jtStartIndex, $jtPageSize);
		}else{
			$result = $dao->get_relation_search($name, $jtSorting, $jtStartIndex, $jtPageSize);
		}
		...
		if($result != null){
			...
			$jtable_result['TotalRecordCount'] = $dao->get_all_count_relations()->fields[0];	
		}else{
			$jtable_result['Result'] = "ERROR";
